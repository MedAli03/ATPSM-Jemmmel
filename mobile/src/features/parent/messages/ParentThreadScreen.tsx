...
export function ParentThreadScreen(): JSX.Element {
  ...
  const messages = useParentThreadMessages(threadId);
  const sendMessage = useSendParentMessage(threadId);
  const markRead = useMarkParentThreadRead(threadId);
  ...
  const flattened = useMemo(() => {
    return messages.data?.pages.flatMap((page) => page.data) ?? [];
  }, [messages.data]);
